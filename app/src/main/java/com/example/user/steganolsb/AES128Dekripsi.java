...
public class AES128Dekripsi {
    private String chiper;
    private String kunci;
    private String[][][] chiperHEX=null;
    private String[][] kunciHEX = new String[4][4];
    private String[][][] roundKey = new String[11][4][4];
    private int panjangChiper;
    private int jumlahBlokChiper;
    ...
    public AES128Dekripsi(String c, String k){

        chiper = c;
        kunci = k;
        panjangChiper=chiper.length();
        jumlahBlokChiper=chiper.length()/16;

        chiperHEX=chiperToMatrixHex(chiper);
    ...
    public String getPesan(){
        String pesan="";
        for(int i=0;i<jumlahBlokChiper;i++){
            for(int y=0; y<4; y++){
                for(int x=0; x<4; x++){
                    pesan+=(char)Integer.parseInt(chiperHEX[i][x][y],16 );
    ...
    private String[][][] chiperToMatrixHex(String string16){
        int c=0;
        String[][][] matrixHEX = new String[jumlahBlokChiper][4][4];
    ...
    private void addRoundKey(int n){
        for(int i=0;i<jumlahBlokChiper;i++){
            for(int y=0; y<4; y++){
                for(int x=0; x<4; x++){
                    chiperHEX[i][y][x] = xorStringHex(chiperHEX[i][y][x],roundKey[n][y][x]);
                }
            }
        }
    }
    ...
    public void subBytesInverse(){
        for(int i=0;i<jumlahBlokChiper;i++){
            for (int y=0;y<4;y++){
                for (int x=0;x<4;x++){
                    chiperHEX[i][y][x] = SBOXINVERSE[ Integer.parseInt(Character.toString(chiperHEX[i][y][x].charAt(0)), 16)][ Integer.parseInt(Character.toString(chiperHEX[i][y][x].charAt(1)), 16)];
                }
            }
        }
    ...
    public void shiftRowsInverse(){
        for(int i=0;i<jumlahBlokChiper;i++){
            chiperHEX[i][1] = geser(chiperHEX[i][1],3);
            chiperHEX[i][2] = geser(chiperHEX[i][2],2);
            chiperHEX[i][3] = geser(chiperHEX[i][3],1);
        }
    }
        ...
        for(int i=0;i<jumlahBlokChiper;i++){
            for(int c=0;c<4;c++){
                kolomHasil[0] =  xor4StringHex   (
                        lookUp(chiperHEX[i][0][c],MATRIXMCINVERSE[0][0]),
                        lookUp(chiperHEX[i][1][c],MATRIXMCINVERSE[0][1]),
                        lookUp(chiperHEX[i][2][c],MATRIXMCINVERSE[0][2]),
                        lookUp(chiperHEX[i][3][c],MATRIXMCINVERSE[0][3])
                );
                kolomHasil[1] =  xor4StringHex   (
                        lookUp(chiperHEX[i][0][c],MATRIXMCINVERSE[1][0]),
                        lookUp(chiperHEX[i][1][c],MATRIXMCINVERSE[1][1]),
                        lookUp(chiperHEX[i][2][c],MATRIXMCINVERSE[1][2]),
                        lookUp(chiperHEX[i][3][c],MATRIXMCINVERSE[1][3])
                );
                kolomHasil[2] =  xor4StringHex   (
                        lookUp(chiperHEX[i][0][c],MATRIXMCINVERSE[2][0]),
                        lookUp(chiperHEX[i][1][c],MATRIXMCINVERSE[2][1]),
                        lookUp(chiperHEX[i][2][c],MATRIXMCINVERSE[2][2]),
                        lookUp(chiperHEX[i][3][c],MATRIXMCINVERSE[2][3])
                );
                kolomHasil[3] =  xor4StringHex   (
                        lookUp(chiperHEX[i][0][c],MATRIXMCINVERSE[3][0]),
                        lookUp(chiperHEX[i][1][c],MATRIXMCINVERSE[3][1]),
                        lookUp(chiperHEX[i][2][c],MATRIXMCINVERSE[3][2]),
                        lookUp(chiperHEX[i][3][c],MATRIXMCINVERSE[3][3])
                );
                chiperHEX[i][0][c] = kolomHasil[0];
                chiperHEX[i][1][c] = kolomHasil[1];
                chiperHEX[i][2][c] = kolomHasil[2];
                chiperHEX[i][3][c] = kolomHasil[3];
            }
        }
    }
