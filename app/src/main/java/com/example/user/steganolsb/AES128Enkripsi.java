...
public class AES128Enkripsi {
    ...
    public AES128Enkripsi(String p, String k){

        pesan = p;
        kunci = k;
        panjangPesan=pesan.length();
        sisaBagi=panjangPesan%16;
        pesan=tambahSpasi(pesan);
        jumlahBlokPesan=pesan.length()/16;

        pesanHEX=pesanToMatrixHex(pesan);
        kunciHEX = stringToMatrixHex(kunci);
    }

    //menambahkan spasi apaila karakter kurang dari 16
    private String tambahSpasi(String string16){
        String pesan=string16;
        if(sisaBagi!=0){//apabila tidak kelipatan 16
            int tambahSpasi=16-sisaBagi;
            for(int i=1;i<=tambahSpasi;i++){
                pesan+=" ";
            }
        }
    ...
    private String[][][] pesanToMatrixHex(String string16){
        int c=0;
