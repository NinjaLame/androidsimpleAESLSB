...
public class AES {
    String pesan;
    String kunci;
    String[][] pesanHEX = new String[4][4];
    String[][] kunciHEX = new String[4][4];
    String[][][] roundKey = new String[11][4][4];
    ...
    static final String[][] SBOXINVERSE =   {
    ...
    static final String[][] MATRIXMCINVERSE =   {
            {"14","11","13","09"},
            {"09","14","11","13"},
            {"13","09","14","11"},
            {"11","13","09","14"}
    ...
    public AES(String p, String k){
        ...
        pesan = p;
        kunci = k;
        pesanHEX = stringToMatrixHex(pesan);
        kunciHEX = stringToMatrixHex(kunci);

        System.out.println("----- ENKRIPSI AES -----");
        System.out.println("Pesan/State : "+pesan);
        System.out.println("Key         : "+kunci);

        System.out.println("");

        System.out.println("Matrix Pesan");
        cetak(pesanHEX);

        System.out.println("Matrix Kunci");
        cetak(kunciHEX);

    }
    public void addRoundKey(int n){
        for(int y=0; y<4; y++){
            for(int x=0; x<4; x++){
                pesanHEX[y][x] = xorStringHex(pesanHEX[y][x],roundKey[n][y][x]);
            }
        }
    ...
    public void subBytes(){
        for (int y=0;y<4;y++){
            for (int x=0;x<4;x++){
                pesanHEX[y][x] = SBOX[ Integer.parseInt(Character.toString(pesanHEX[y][x].charAt(0)), 16)][ Integer.parseInt(Character.toString(pesanHEX[y][x].charAt(1)), 16)];
            }
        }
    ...
    public void shiftRows(){
        pesanHEX[1] = geser(pesanHEX[1],1);
        pesanHEX[2] = geser(pesanHEX[2],2);
        pesanHEX[3] = geser(pesanHEX[3],3);
    ...
    public void mixColumn(){
        ...
        for(int c=0;c<4;c++){
            kolomHasil[0] =  xor4StringHex   (
                    lookUp(pesanHEX[0][c],MATRIXMC[0][0]),
                    lookUp(pesanHEX[1][c],MATRIXMC[0][1]),
                    lookUp(pesanHEX[2][c],MATRIXMC[0][2]),
                    lookUp(pesanHEX[3][c],MATRIXMC[0][3])
            );
            kolomHasil[1] =  xor4StringHex   (
                    lookUp(pesanHEX[0][c],MATRIXMC[1][0]),
                    lookUp(pesanHEX[1][c],MATRIXMC[1][1]),
                    lookUp(pesanHEX[2][c],MATRIXMC[1][2]),
                    lookUp(pesanHEX[3][c],MATRIXMC[1][3])
            );
            kolomHasil[2] =  xor4StringHex   (
                    lookUp(pesanHEX[0][c],MATRIXMC[2][0]),
                    lookUp(pesanHEX[1][c],MATRIXMC[2][1]),
                    lookUp(pesanHEX[2][c],MATRIXMC[2][2]),
                    lookUp(pesanHEX[3][c],MATRIXMC[2][3])
            );
            kolomHasil[3] =  xor4StringHex   (
                    lookUp(pesanHEX[0][c],MATRIXMC[3][0]),
                    lookUp(pesanHEX[1][c],MATRIXMC[3][1]),
                    lookUp(pesanHEX[2][c],MATRIXMC[3][2]),
                    lookUp(pesanHEX[3][c],MATRIXMC[3][3])
            );
            pesanHEX[0][c] = kolomHasil[0];
            pesanHEX[1][c] = kolomHasil[1];
            pesanHEX[2][c] = kolomHasil[2];
            pesanHEX[3][c] = kolomHasil[3];
        }
    ...
    public void addRoundKeyInverse(int n){
        for(int y=0; y<4; y++){
            for(int x=0; x<4; x++){
                pesanHEX[y][x] = xorStringHex(pesanHEX[y][x],roundKey[n][y][x]);
            }
        }
        System.out.println("Add Round Key Inverse");
        cetak(pesanHEX);
    }
    public void subBytesInverse(){
        for (int y=0;y<4;y++){
            for (int x=0;x<4;x++){
                pesanHEX[y][x] = SBOXINVERSE[ Integer.parseInt(Character.toString(pesanHEX[y][x].charAt(0)), 16)][ Integer.parseInt(Character.toString(pesanHEX[y][x].charAt(1)), 16)];
            }
        }
        System.out.println("After Sub Bytes Inverse");
        cetak(pesanHEX);
    }
    public void shiftRowsInverse(){
        pesanHEX[1] = geser(pesanHEX[1],3);
        pesanHEX[2] = geser(pesanHEX[2],2);
        pesanHEX[3] = geser(pesanHEX[3],1);
        System.out.println("After Shift Rows Inverse");
        cetak(pesanHEX);
    }
    public void mixColumnInverse(){
        ...
        for(int c=0;c<4;c++){
            kolomHasil[0] =  xor4StringHex   (
                    lookUp(pesanHEX[0][c],MATRIXMCINVERSE[0][0]),
                    lookUp(pesanHEX[1][c],MATRIXMCINVERSE[0][1]),
                    lookUp(pesanHEX[2][c],MATRIXMCINVERSE[0][2]),
                    lookUp(pesanHEX[3][c],MATRIXMCINVERSE[0][3])
            );
            kolomHasil[1] =  xor4StringHex   (
                    lookUp(pesanHEX[0][c],MATRIXMCINVERSE[1][0]),
                    lookUp(pesanHEX[1][c],MATRIXMCINVERSE[1][1]),
                    lookUp(pesanHEX[2][c],MATRIXMCINVERSE[1][2]),
                    lookUp(pesanHEX[3][c],MATRIXMCINVERSE[1][3])
            );
            kolomHasil[2] =  xor4StringHex   (
                    lookUp(pesanHEX[0][c],MATRIXMCINVERSE[2][0]),
                    lookUp(pesanHEX[1][c],MATRIXMCINVERSE[2][1]),
                    lookUp(pesanHEX[2][c],MATRIXMCINVERSE[2][2]),
                    lookUp(pesanHEX[3][c],MATRIXMCINVERSE[2][3])
            );
            kolomHasil[3] =  xor4StringHex   (
                    lookUp(pesanHEX[0][c],MATRIXMCINVERSE[3][0]),
                    lookUp(pesanHEX[1][c],MATRIXMCINVERSE[3][1]),
                    lookUp(pesanHEX[2][c],MATRIXMCINVERSE[3][2]),
                    lookUp(pesanHEX[3][c],MATRIXMCINVERSE[3][3])
            );
            pesanHEX[0][c] = kolomHasil[0];
            pesanHEX[1][c] = kolomHasil[1];
            pesanHEX[2][c] = kolomHasil[2];
        ...
        System.out.println("After Mix Column Inverse");
        cetak(pesanHEX);
    }
    ...
    private void cetakPesan(){
        for(int y=0; y<4; y++){
            for(int x=0; x<4; x++){
                System.out.print((char)Integer.parseInt(pesanHEX[x][y], 16 ));
            }
        }
        System.out.println("");
    }
    private void cetak(String[][] matrix){
        for(int y=0; y<4; y++){
            for(int x=0; x<4; x++){
...
//        AES aes = new AES("Noveline Fauziah","belajar enkripsi");
////        AES128 aes = new AES128("2Cö¨Z011¢à74","+~(®Ò¦«÷	ÏO<");
...
//        aes.cetak(aes.pesanHEX);
//        System.out.println("=================================================");
//
//        aes.addRoundKey(10);
//        for(int c=9;c>0;c--){
//            System.out.println("--- Round " + (c+1) + " --- ");
//            aes.shiftRowsInverse();
//            aes.subBytesInverse();
//            aes.addRoundKey(c);
//            aes.mixColumnInverse();
//        }
//        aes.shiftRowsInverse();
//        aes.subBytesInverse();
//        aes.addRoundKey(0);
//
//        System.out.println("DEKRIP ASCII : ");
//        aes.cetakPesan();
//        System.out.println("DEKRIP HEX : ");
//        aes.cetak(aes.pesanHEX);
//        System.out.println("Noveline Fauziah".length());
//    }
}
